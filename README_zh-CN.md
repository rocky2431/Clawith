...
### 🏛️ 组织级管控
- **多租户 RBAC** — 3 层租户隔离（中间件 + contextvar + PostgreSQL RLS）+ 角色权限控制
- **渠道集成** — 每个 Agent 可拥有独立的 Slack、Discord、飞书/Lark、钉钉、企业微信或 Teams 机器人身份
- **用量控制** — 每用户消息限额、LLM 调用上限、Agent 存活时间、Redis 滑动窗口限流
- **审批工作流** — 危险操作标记，需人工审核后方可执行
- **审计日志 & 知识库** — 哈希链审计追踪 + 组织共享上下文自动注入
- **密钥加密** — Fernet 信封加密，LLM API Key 静态加密存储
- **配置版本控制** — Agent 设置完整快照历史，支持一键回滚
- **Agent 分类** — 4 类（内部系统、内部租户、外部网关、外部 API）× 3 区（标准、受限、公开）安全模型
- **功能开关** — 租户/用户/百分比级别开关，Redis 缓存
- **API 版本化** — `/api/v1/` 端点前缀，向后兼容双挂载
...
### ⚙️ 执行引擎
- **中间件链架构** — 可插拔的执行管线，支持上下文注入、记忆提取、工具可靠性保障等
- **L0/L1/L2 分层上下文** — 自适应上下文窗口管理（系统提示 → 近期记忆 → 完整历史）
- **结构化记忆** — 基于事实的长期记忆，支持相关性衰减和去抖提取
- **工具可靠性** — 所有工具调用自动重试、熔断和超时保护
- **Redis Streams 事件总线** — 持久化、有序的事件投递，替代内存 pub/sub
